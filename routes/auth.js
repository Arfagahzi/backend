...
router.post('/login', [helper.hasAuthFields, helper.isPasswordAndUserMatch], (req, res) => {
    ...
    getUserData (req.body.email, function(resUser) {
        if (resUser != null) {
            console.log(`email = ${resUser.firstname}`);
            res.json({token: token, auth: true, userId: resUser.userId, email: req.body.email, firstname: resUser.firstname, lastname: resUser.lastname ,photoUrl: resUser.photoUrl });
            return;
        }
    });
...
async function getUserData(email, callback) {
    const user = await helper.database.table('users').filter({$or:[{ email : email },{ username : email }]}).get();
    var resUser = {};
    if (user) {
        resUser = {
            "userId": user.id,
